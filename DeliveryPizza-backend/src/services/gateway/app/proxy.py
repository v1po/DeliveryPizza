...
class ServiceProxy:
    ...
    async def proxy_request(
        ...
        if auth_header and "x-authorization" not in {k.lower() for k in headers}:
            headers["X-Authorization"] = auth_header

        # Ensure internal service receives correct Host header
        parsed_host = service_url.split("://", 1)[-1]
